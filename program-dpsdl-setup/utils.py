...
def create_voltage_lines_group(title: str,
                               vin_label: str,
                               vin_name: str,
                               ...
                               button_text: str = 'GET'):
    """
    전압 표를 깔끔하게 구성하는 전용 레이아웃 생성기.
    형태: [Label] [IN] [OUT]
    첫 줄은 Vin만 표시하고, 이후 줄부터 Line1~N의 IN/OUT을 한 줄에 배치합니다.

    Args:
        title: 그룹 박스 제목
        vin_label: Vin 레이블 텍스트 (예: 'Vin DC:')
        vin_name: Vin 입력 위젯 이름 키 (예: 'vin_dc')
        lines: 각 라인의 (표시라벨, in_name, out_name) 튜플 리스트
        input_width: 입력 박스 폭
        read_only: 입력 박스 읽기 전용 여부
        button_text: 하단 우측 버튼 텍스트 ('GET' 권장)

    Returns:
        tuple[QGroupBox, dict]: 생성된 그룹과 위젯 딕셔너리
    """
    group_box = QGroupBox(title)
    main_vbox = QVBoxLayout()
    grid = QGridLayout()
    ...
    header_blank = QLabel("")
    header_in = QLabel("IN")
    header_out = QLabel("OUT")
    header_in.setAlignment(Qt.AlignmentFlag.AlignCenter)
    header_out.setAlignment(Qt.AlignmentFlag.AlignCenter)
    grid.addWidget(header_blank, 0, 0)
    grid.addWidget(header_in,   0, 1)
    grid.addWidget(header_out,  0, 2)

    row = 1

    # Vin DC 행 (Label + IN 필드만)
    vin_lbl = QLabel(vin_label)
    vin_edit = QLineEdit()
    vin_edit.setFixedWidth(input_width)
    vin_edit.setReadOnly(read_only)
    grid.addWidget(vin_lbl,   row, 0)
    grid.addWidget(vin_edit,  row, 1)
    # OUT 자리는 비워둠
    widgets[vin_name] = vin_edit
    row += 1

    # Line1~N 행들
    for line_label, in_name, out_name in lines:
        lbl = QLabel(line_label)
        in_edit = QLineEdit(); in_edit.setFixedWidth(input_width); in_edit.setReadOnly(read_only)
        out_edit = QLineEdit(); out_edit.setFixedWidth(input_width); out_edit.setReadOnly(read_only)

        grid.addWidget(lbl,      row, 0)
        grid.addWidget(in_edit,  row, 1)
        grid.addWidget(out_edit, row, 2)

        widgets[in_name] = in_edit
        widgets[out_name] = out_edit
        row += 1
    ...
    grid.setColumnStretch(3, 1)  # 여유 공간은 오른쪽으로
...
def create_dynamic_group(title, layout_config, options_data=None, input_width=60 ):
    ...
    for item in layout_config:
        ...
        if item_type == 'input_pair':
            ...
            time_edit = QLineEdit()
            time_edit.setPlaceholderText(item.get('placeholder', 'HH:mm'))
            time_edit.setFixedWidth(item.get('time_width', 70))
            # 24시간 HH:mm 검증 (PyQt6: QRegularExpression, PyQt5: QRegExp)
