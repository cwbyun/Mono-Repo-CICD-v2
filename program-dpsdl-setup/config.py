...
class ConfigTab(QWidget):
    def __init__(self, parent):
        ...
        layout1_config = [
                {'type': 'input_pair', 'label':'Voltage TOL:', 'name':'vtol'},
    ...
    def voltage_limit_set_btn( self ):
        ...
        try:
            vtol_str  = extract_number_from_text(self.widgets1['vtol'].text())
            retry_str = extract_number_from_text(self.widgets1['retry'].text())


            vtol_val  = int( float(vtol_str) )
        ...
        data_str = str(vtol_val).zfill(2) + str(retry_val)
        print("ret = ", data_str)
    ...
    def voltage_limit_get_btn(self):
        ...
        try :
            ...
            return_tuple = parse_by_lengths( ans, "21" )

            vtol = int(return_tuple[0])
            retry = int(return_tuple[1])


            # 칸 오른쪽 정렬
            self.widgets1['vtol'].setAlignment(Qt.AlignmentFlag.AlignRight)
            self.widgets1['retry'].setAlignment(Qt.AlignmentFlag.AlignRight)

            self.widgets1['vtol'].setText(str(vtol)+" %");
