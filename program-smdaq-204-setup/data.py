...
class DataTab(QWidget):
    def __init__(self, parent):
        ...
        self.loading_label = QLabel("로딩 중... 잠시만 기다리세요.")
        ...
        main_layout.addWidget(scroll)
        self.widgets1['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Data List 로딩 중...", self.data_list_get_btn)
        )
        #self.widgets1['dl_25_get_btn'].clicked.connect( self.data_list_dl25_get_btn )

        self.widgets2['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Latest DATA 로딩 중...", self.latest_data_get_btn)
        )
        self.widgets3['get_btn'].clicked.connect(
            lambda: self.run_with_busy("All DATA 로딩 중...", self.all_data_get_btn)
        )


        self.widgets4['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Data in Folder 로딩 중...", self.data_in_folder_get_btn)
        )


        self.widgets5['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Folder Number 로딩 중...", self.folder_number_get_btn)
        )
        self.widgets6['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Folder Range 로딩 중...", self.folder_in_range_get_btn)
        )
        self.widgets7['get_btn'].clicked.connect(
            lambda: self.run_with_busy("One Data List 로딩 중...", self.one_data_list_get_btn)
        )
        self.widgets8['get_btn'].clicked.connect(
            lambda: self.run_with_busy("SD Card File List 로딩 중...", self.sd_card_file_list_get_btn)
        )
        self.widgets9['get_btn'].clicked.connect(
            lambda: self.run_with_busy("SD Card File Data 로딩 중...", self.sd_card_file_data_get_btn)
        )
        self.widgets10['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Number Data 로딩 중...", self.number_data_get_btn)
        )
        self.widgets11['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Alarm Data 로딩 중...", self.alarm_data_get_btn)
        )
        self.widgets12['get_btn'].clicked.connect(
            lambda: self.run_with_busy("Stop Reading 로딩 중...", self.stop_reading_get_btn)
        )
    ...
    def set_busy(self, busy: bool, message=None):
        self._busy = busy
        if message:
    ...
    def run_with_busy(self, message: str, fn):
        if self._busy:
            return
